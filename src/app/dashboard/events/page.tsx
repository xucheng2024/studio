  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid gap-4">
        {(events ?? []).filter((e) => String(e.studio_id) === studioId).map((e) => {
      ...
      {!(events ?? []).filter((e) => String(e.studio_id) === studioId).length ? (
        <div className={`mt-2 ${ui.emptyState}`}>
          <p className={`text-sm ${ui.muted}`}>No events yet.</p>
        </div>
      ) : null}
    </div>
  );
