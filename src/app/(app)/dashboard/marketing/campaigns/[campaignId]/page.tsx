...
export default async function MarketingCampaignReportPage({ params }: Props) {
  ...
  return <div className="space-y-6">
    <div><Link className="text-sm underline" href={`/dashboard/marketing?studio_id=${campaign.studio_id}${campaign.location_id ? `&location_id=${campaign.location_id}` : ""}&tab=campaigns`}>← Marketing</Link><h1 className={ui.h1}>{campaign.name}</h1><p className={ui.muted}>{campaign.audience_type} · <span className="capitalize">{campaign.status}</span></p></div>
