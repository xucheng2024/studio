...
import { createMarketingCampaignAction, sendMarketingTestEmailAction } from "@/app/(app)/dashboard/actions";
...
export default async function MarketingPage({ searchParams }: Props) {
  ...
  if (!user) return null;
  const { studioIds, selectedStudioId, selectedLocationId, accessibleLocationIds } = await getDashboardScopeForRoles({ userId: user.id, email: user.email, studioId: sp.studio_id ?? null, locationId: sp.location_id ?? null }, ["owner", "manager"]);
  ...
  const [{ data: studio }, { data: locations }, { data: campaigns }] = await Promise.all([
    ...
    admin.from("marketing_campaigns").select("id, name, audience_type, status, recipient_snapshot_at, created_at").eq("studio_id", studioId).order("created_at", { ascending: false }).limit(20),
  ]);
  if (!studio) return <p className={ui.muted}>Select a studio to manage marketing.</p>;
  const locationId = selectedLocationId ?? "";
  return <div className="space-y-6">
    <div><h1 className={ui.h1}>Marketing</h1><p className={ui.muted}>Create consent-safe Email drafts. Sending and reporting arrive in the next phase.</p></div>
    <DashboardLocationFilter selectedStudioId={studioId} selectedLocationId={selectedLocationId} locations={(locations ?? []).filter((location) => accessibleLocationIds.includes(location.id))} />
