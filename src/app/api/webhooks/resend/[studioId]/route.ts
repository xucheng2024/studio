...
import { getStudioResendSecrets } from "@/lib/studio-email";
...
const studioIdPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
...
export async function POST(
  ...
  if (!studioIdPattern.test(studioId)) {
    return NextResponse.json({ error: "invalid_studio" }, { status: 404 });
  }

  const secrets = await getStudioResendSecrets(studioId);
  if (!secrets) {
    console.error("[resend-webhook] studio secret missing", { studioId });
    return NextResponse.json({ error: "resend_webhook_not_configured" }, { status: 401 });
  }
  ...
  if (!recipient || recipient.studio_id !== studioId) return NextResponse.json({ ok: true, ignored: true });
