...
import {
  ...
  markSubscriptionCanceledFromRemoteNotFound,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (msg === "hitpay_recurring_not_found" && await markSubscriptionCanceledFromRemoteNotFound(admin, row, new Date(nowIso))) {
  ...
  await applyRecurringSubscriptionStatus(admin, row, {
    recurringStatusRaw: remote.status,
    gatewayPayload,
    eventKind: "provider_sync",
    occurredAt: nowIso,
  });
