...
| Q2 多门店集中管理 | 🟡 已上线 / 待申请截图 | 服务发布、员工多门店、业务归店与 Reports 单店/全店汇总已进入生产。申请截图与越权演示后填 Yes。 | 申请截图后 Yes |
| Q3 Dashboard/Reports | 🟡 已上线 / 待申请截图 | 四图 Dashboard 与 Appointment/Sales/Retention/FOV/员工/套餐递延报表已进入生产。Inventory/Loyalty 未启用填 0。Surgery 2026-08 无销售是真数据。 | 申请截图后 Yes |
...
| Q13 收据和付款 | ✅ 基本满足 | 付款记录、编号、PDF Invoice 和退款可复用。已付款 POS 明细可 Preview/Send invoice。找零 UI 与独立 Credit Note 号段仍未做。 | Yes |
| Q14 数字支付 | ✅ 基本满足 | 继续使用 HitPay；保留现金记录。申请材料只列出生产环境真实启用、可现场完成的 HitPay 支付方式。 | Yes |
| Q15 POS | 🟡 已上线 / 待申请截图 | in-house POS 已进入生产：客户、多服务/商品/套餐、员工、折扣、Cash/HitPay、收据、退款、作废、门店归属和日结。已付款 POS 可发送 PDF invoice。找零 UI 仍不在本批。 | 申请截图后 Yes |
...
| Q17 HR Management | 🟡 已上线 / 待申请截图 | 员工主档、佣金、PAY-01 档案/规则、PAY-02 四状态跑批和 PAY-03 Payslip/报表已上线。已发布工资单可 Email PDF，不在 Finalise 时自动发送。顾问签字不是硬前置。 | 申请截图后 Yes |
...
| Q28 Business Data Extraction | 🟡 已上线 / 待申请截图 | Deferred、Sales、Customers、Packages 与 Payroll/Commission 四格式导出已进入生产。申请截图与样例文件后填 Yes。 | 申请截图后 Yes |
