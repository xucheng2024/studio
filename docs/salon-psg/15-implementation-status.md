...
| Phase 1 | CRM-01 Sensitive Customer Data | 已实现/待验证 | FND-02 | 已完成敏感资料四张表、append-only Consent/Access Audit、RLS+service_role-only RPC、Server-only scope 封装、客户列表/详情分区与安全提醒接口；`test:crm01-db`、`npx tsc --noEmit`、相关 ESLint、`git diff --check` 通过，待补真实环境角色矩阵+浏览器+移动端回归后升为已验证 |
...
1. FND-04 与 CRM-01 当前均为“已实现/待验证”；建议先在真实环境完成角色矩阵、浏览器端与移动端回归，再统一升为“已验证/待上线”。
2. APT-02 当前为“已验证/待上线”；可进入上线窗口与灰度发布准备。
3. CRM-02、APT-04、MKT-01 等依赖 CRM-01 的任务可基于新敏感资料与 Consent 契约启动设计。
