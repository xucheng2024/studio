...
Choose GitHub Actions when you want the simplest setup and do not need to maintain a server. The repository already supplies **Free cloud UAT** under Actions; select one flow, or select **all** to run the seven isolated flows in parallel. Each job creates a fresh x64 Ubuntu runner, starts Docker-backed local Supabase, runs its browser verifier, and cleans up.

Select **all-batched** when GitHub Actions minutes matter more than elapsed time. It starts the disposable Supabase environment once, runs the seven audited flows sequentially with isolated fixture identities, then cleans up once. It is slower to receive the complete result than **all**, but avoids six repeated service startups and migration replays.
...
For a few hours of active development, run only the flow affected by the latest change. A newer run for the same branch and flow automatically cancels its older in-progress run, so stale checks do not form a queue. Different flows can run concurrently; selecting **all** fans out to all seven. Parallel runs reduce wall-clock time but consume roughly the same GitHub Actions minutes as running the flows one after another.
