...
select set_config('pos02_uat.studio_id', :'pos02_uat_studio_id', false);
select set_config('pos02_uat.location_id', :'pos02_uat_location_id', false);
select set_config('pos02_uat.customer_user_id', :'pos02_uat_customer_user_id', false);
select set_config('pos02_uat.customer_id', :'pos02_uat_customer_id', false);
select set_config('pos02_uat.service_id', :'pos02_uat_service_id', false);
select set_config('pos02_uat.sale_id', :'pos02_uat_sale_id', false);
select set_config('pos02_uat.sale_item_id', :'pos02_uat_sale_item_id', false);
select set_config('pos02_uat.payment_id', :'pos02_uat_payment_id', false);

do $$
declare
  v_studio uuid := current_setting('pos02_uat.studio_id')::uuid;
  v_location uuid := current_setting('pos02_uat.location_id')::uuid;
  ...
  v_customer_user uuid := current_setting('pos02_uat.customer_user_id')::uuid;
  v_customer uuid := current_setting('pos02_uat.customer_id')::uuid;
  v_service uuid := current_setting('pos02_uat.service_id')::uuid;
  v_sale uuid := current_setting('pos02_uat.sale_id')::uuid;
  v_sale_item uuid := current_setting('pos02_uat.sale_item_id')::uuid;
  v_payment uuid := current_setting('pos02_uat.payment_id')::uuid;
...
begin
  if exists (
    ...
    raise exception 'POS-02 local fixture requires exact local Auth identities';
  end if;
  ...
  insert into public.user_profiles (id, email, full_name, role) values
    (v_owner, 'pos-local-owner@example.test', 'POS-02 local owner', 'member'),
    (v_manager, 'pos-local-manager@example.test', 'POS-02 local manager', 'member'),
    (v_frontdesk, 'pos-local-frontdesk@example.test', 'POS-02 local frontdesk', 'member'),
    (v_instructor, 'pos-local-instructor@example.test', 'POS-02 local instructor', 'member')
  on conflict (id) do update set email = excluded.email, full_name = excluded.full_name;

  insert into public.studios (id, owner_id, name, public_slug, contract_status)
  values (v_studio, v_owner, 'POS-02 local UAT', 'pos02-uat-' || left(replace(v_studio::text, '-', ''), 12), 'active');

  insert into public.locations (id, studio_id, name, is_active)
  values (v_location, v_studio, 'POS-02 Local', true);
  ...
  values (v_customer, v_studio, v_customer_user, 'POS-02 local customer', v_customer_email, 'active', 'frontdesk', v_location);

  insert into public.studio_services (id, studio_id, title, price, currency, is_active)
  values (v_service, v_studio, 'POS-02 local service', 100, 'SGD', true);
  ...
  insert into public.pos_sales (
    ...
    100, 0, 0, 100, 'POS-02 local cash transaction', now(), now(), v_owner, v_owner
  );
  ...
  insert into public.pos_sale_items (
    ...
    v_sale_item, v_sale, v_studio, v_location, 1, 'service', v_service, 'POS-02 local service',
  ...
  insert into public.payments (
    ...
    'frontdesk', 'pos_sale', 'pending', 'POS02-UAT-' || left(replace(v_payment::text, '-', ''), 16), 'single', 0
  );
end $$;

select 'pos02_uat_fixture_ok' as result;
