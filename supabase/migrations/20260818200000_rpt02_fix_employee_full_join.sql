-- RPT-02: replace employee FULL JOIN ... IS NOT DISTINCT FROM (Postgres 0A000)
-- with UNION ALL so get_rpt01_reporting_facts can run on hosted Postgres.
...
begin
  ...
  sale_by_day as (
    ...
    from (
      select
        coalesce(paid_date, refund_date) as day,
        coalesce(sum(gross_in_range - refund_in_range) filter (where item_type = 'service'), 0) as service_net,
        coalesce(sum(gross_in_range - refund_in_range) filter (where item_type = 'product'), 0) as retail_net
      from sale_in_range
      where coalesce(paid_date, refund_date) is not null
      group by coalesce(paid_date, refund_date)
    ) rows
  ),
  yoy_items as (
    select
  ...
  employee_rows as (
    ...
    from (
      select
        employee_id,
        coalesce(max(employee_label), 'Unassigned') as employee_label,
        sum(completed_services) as completed_services,
        sum(net_service_sales) as net_service_sales,
        sum(net_commission) as net_commission
      from (
        select
          employee_id,
          employee_label,
          count(*)::numeric as completed_services,
          0::numeric as net_service_sales,
          0::numeric as net_commission
        from appt
        where status = 'completed'
        ...
        select
          ...
          0::numeric,
          coalesce(sum(gross_in_range - refund_in_range), 0)::numeric,
          0::numeric
  ...
  select jsonb_build_object(
    ...
    'appointment_outcome', (select payload from appt_outcome) || jsonb_build_object(
      ...
      'by_day', (select payload from appt_by_day)
    ),
    'sales', (select payload from sale_totals) || jsonb_build_object(
      ...
      'by_day', (select payload from sale_by_day),
