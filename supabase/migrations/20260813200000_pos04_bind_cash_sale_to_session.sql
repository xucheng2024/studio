-- POS-04 Batch 3: bind cash sale completion to active cash session (strict mode).
...
declare
  ...
  v_open_session public.pos_cash_sessions;
  v_now timestamptz;
  v_result jsonb;
begin
  ...
  begin
    ...
    if v_sale_before.status = 'paid' and v_payment_before.status = 'paid' then
      v_result := jsonb_build_object(
        ...
        'cash_session_id', v_payment_before.cash_session_id,
    ...
    where s.studio_id = p_studio_id
      and s.location_id = v_sale_before.location_id
      and s.status = 'open'
    ...
    if not found then
      raise exception 'no open cash session for location %', v_sale_before.location_id using errcode = '23514';
    end if;
    ...
    set status = 'paid',
        ...
        cash_session_id = coalesce(v_payment_before.cash_session_id, v_open_session.id)
    ...
    perform public.record_strong_audit(
      ...
      p_after_state := jsonb_build_object(
        ...
        'cash_session_id', v_payment_after.cash_session_id
      ),
...
revoke all on function public.complete_pos_cash_sale(uuid, text, uuid, uuid, text, text)
  from public, anon, authenticated;

grant execute on function public.complete_pos_cash_sale(uuid, text, uuid, uuid, text, text)
  to service_role;
