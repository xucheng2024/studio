-- POS-03 follow-up: align lock order to sale -> payment
-- Reason: avoid payment <-> sale deadlock against fulfill/commission paths.
...
begin
  ...
  if p_sale_id is not null then
    v_lookup_sale_id := p_sale_id;
  else
    select p.pos_sale_id
      into v_lookup_sale_id
    from public.payments p
    where p.id = p_payment_id
      and p.studio_id = p_studio_id;

    if v_lookup_sale_id is null then
  ...
  where s.id = v_lookup_sale_id
    and s.studio_id = p_studio_id
  for update;

  if not found then
    raise exception 'POS sale % not found in studio %', v_lookup_sale_id, p_studio_id using errcode = 'P0002';
  end if;
  ...
  if v_payment_before.pos_sale_id <> v_sale_before.id then
    raise exception 'payment % is linked to sale %, not %', v_payment_before.id, v_payment_before.pos_sale_id, v_sale_before.id
      using errcode = '23514';
  end if;

  if p_sale_id is not null and v_sale_before.id <> p_sale_id then
    raise exception 'sale % does not match requested sale %', v_sale_before.id, p_sale_id
      using errcode = '23514';
  end if;
